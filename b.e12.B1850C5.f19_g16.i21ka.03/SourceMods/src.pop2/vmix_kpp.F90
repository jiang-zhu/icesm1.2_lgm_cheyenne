...
!-----------------------------------------------------------------------
        ...
        endwhere
!! --------------------------
!! Solving instability problem in Denmark strait by forcing tidal mixing in the
!! lowest 4 KMT levels.
!! Gokhan 19nov2010 and nanr and pdn and ruza
!! --------------------------
        if (.not. lccsm_control_compatible) then   ! this step breaks backwards compatibility
!!        where ( k > 2  .and.  ( k == KMT(:,:,bid)-1  .or.  k == KMT(:,:,bid)-2 ) )
          where ( k > 6  .and.  ( k == KMT(:,:,bid)-1  .or.  k == KMT(:,:,bid)-2 .or.  &
                                  k == KMT(:,:,bid)-3  .or.  k == KMT(:,:,bid)-4 .or.  &
                                  k == KMT(:,:,bid)-5  .or.  k == KMT(:,:,bid)-6 ) )
